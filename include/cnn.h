...
void loadDataset(float input[INPUT_ROWS_1][INPUT_COLS_1],
                 const char *filename) {
...
void initLeNet5(LeNet5 *model) {
  FILE *file = fopen("../extern/parameters.txt", "rb");
  if (file == NULL) {
    printf("Failed to open the file.\n");
    return;
  }
...
void layer_7_fc(float input_layer[NUM_FILTERS_6], LeNet5 *model,
                float output_layer[NUM_FILTERS_7]) {
...
void layer_6_fc(float input_layer[NUM_FILTERS_5], LeNet5 *model,
                float output_layer[NUM_FILTERS_6]) {
...
void layer_5_conv_flat(
    float input_layer[NUM_FILTERS_4][OUTPUT_ROWS_4][OUTPUT_COLS_4],
    LeNet5 *model, float output_layer[NUM_FILTERS_5]) {
  // printf("Input dim = 1x%dx%dx%d . Output dim = 1x%d", NUM_FILTERS_4,
  // OUTPUT_ROWS_4, OUTPUT_COLS_4, NUM_FILTERS_5);
...
void layer_4_subsampling(
    float input_layer[NUM_FILTERS_3][OUTPUT_ROWS_3][OUTPUT_COLS_3],
    LeNet5 *model,
    float output_layer[NUM_FILTERS_4][OUTPUT_ROWS_4][OUTPUT_COLS_4]) {
...
void layer_3_conv(
    float input_layer[NUM_FILTERS_2][OUTPUT_ROWS_2][OUTPUT_COLS_2],
    LeNet5 *model,
    float output_layer[NUM_FILTERS_3][OUTPUT_ROWS_3][OUTPUT_COLS_3]) {
  for (int k = 0; k < NUM_FILTERS_3; k++) {
        ...
        for (int c = 0; c < NUM_FILTERS_2; c++) {
          for (int m = 0; m < KERNEL_SIZE_3; m++) {
            for (int n = 0; n < KERNEL_SIZE_3; n++) {
              output_layer[k][i][j] +=
                  input_layer[c][i + m][j + n] * model->weights2[k][c][m][n];
            }
          }
        }
...
void layer_2_subsampling(
    float input_layer[NUM_FILTERS_1][OUTPUT_ROWS_1][OUTPUT_COLS_1],
    LeNet5 *model,
    float output_layer[NUM_FILTERS_2][OUTPUT_ROWS_2][OUTPUT_COLS_2]) {
...
void layer_1_conv(
    float input_layer[INPUT_ROWS_1][INPUT_COLS_1], LeNet5 *model,
    float output_layer[NUM_FILTERS_1][OUTPUT_ROWS_1][OUTPUT_COLS_1]) {
      ...
      for (int input_col = 0; input_col < OUTPUT_COLS_1; input_col++) {
        ...
        for (int kernel_row = 0; kernel_row < KERNEL_SIZE_1; kernel_row++) {
          for (int kernel_col = 0; kernel_col < KERNEL_SIZE_1; kernel_col++) {
            sum += input_layer[input_row + kernel_row][input_col + kernel_col] *
        ...
        output_layer[filter_no][input_row][input_col] =
            tanh((sum + model->biases1[filter_no]));
      }
    }
  }
}
